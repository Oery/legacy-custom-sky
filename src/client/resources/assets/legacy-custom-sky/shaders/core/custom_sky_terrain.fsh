...
// Copy of vanilla's core/terrain.fsh (net.minecraft.client.renderer.RenderPipelines
...
uniform sampler2D CustomSkyEnvMap;
...
// south(+Z)=0, east(+X)=90deg, matching CustomSkyLayer.colorTowardDirection's
// atan2(x, z) convention, so texel (u,v) in CustomSkyEnvMap lines up with the same
// direction CustomSkyEnvironmentMap baked it from.
vec2 customSkyEquirectUv(vec3 direction) {
    vec3 d = normalize(direction);
    float azimuth = atan(d.x, d.z);
    float elevation = asin(clamp(d.y, -1.0, 1.0));
    return vec2(azimuth / (2.0 * 3.14159265) + 0.5, 0.5 - elevation / 3.14159265);
}
